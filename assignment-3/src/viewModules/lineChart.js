...
function LineChart(){
	...
	function exportFunction(data, rootDOM, key){
		...
		const axisY = d3.axisLeft()
			.scale(scaleY)
			.tickSize(-innerWidth)//draw the grid line
			.ticks(3)//the number of grid lines
		...
		//Event handling-hover over function
		plot
			.select('.mouse-target')
	...
	exportFunction.onChangeYear = function(callback){
		//cb is just a function that does console.log
		yearChangeCallback = callback; 
		return this;
	}
