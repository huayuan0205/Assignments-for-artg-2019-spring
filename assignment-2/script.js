...
Promise.all([
		...
		let migration_origin_by_country = d3.nest() //COMPLETE HERE
											...
											.map(a =>{
												return{
													key:a.key,
													total_value:d3.sum(a.values,function(d){
														return d.value;
													})
												}		
											});
		...
		migration_origin_by_country = migration_origin_by_country.map(d =>{
			const codes = countryCode.get(d.key);
			const metadata_value = metadataMap.get(codes);
		
		if(metadata_value){
			d.lngLat = metadata_value.lngLat;
			d.name_display = metadata_value.name_display;
		}else{
			d.lngLat = undefined;//do not use 'undefined', otherwise the filiter of lnglat will return ture in every case
		}
			return d;
		})
		console.group('migration_origin_by_country');
...
function drawCartogram(rootDom, data){
	...
	const nodes = plot.selectAll('.node')
					.data(data, d => d.key);//match each node 1 to 1 with a data array
					//d =>d.key here means bond each node by d.key, but will also brings other properties of data!
	const nodesEnter = nodes.enter()// it's a empty selction now, the function of .enter is create a space for each node and put data into the space
						    .append('g')
							.attr('class','node');//nodesEnter is a selction pointing to each node

	nodesEnter.append('circle');
	nodesEnter.append('text')
		.attr('text-anchor', 'middle');

	nodes.merge(nodesEnter)//merge is taking 2 selections and merging them to 1 selection
						  //so now, nodes is a selection pointing to 234 'g' element
			   .filter(d => d.lngLat)//refers to, filter the data in which lnglat is in the format of number
			   .attr('transform',function(d){
			   		const xy = projection(d.lngLat);
			   		return `translate(${xy[0]},${xy[1]})`;
			   });

	//!!!to see what data each node is bond to
	nodes.each(d => console.log(d));

	nodes.merge(nodesEnter)
			   .select('circle')
			   // .attr('cx', d => projection(d.lngLat)[0])
			   // .attr('cy', d => projection(d.lngLat)[1])
			   .attr('r',d => scaleSize(d.total_value))
   	...
   	nodes.merge(nodesEnter)
		.select('text')
		.filter(d => d.total_value > 1000000)
		.text(d => d.name_display)
		.style('font-family', 'sans-serif')
		.style('font-size', '10px');

	nodesExit = nodes.exit().remove();//do sth. with the surplus space('g')
